...
public class BenchClient {
     ...
     */
    public void connect(String truststorePath, String truststorePassword, int numMessages, String messagePrefix) throws Exception {
        ...
        try (SSLSocket socket = (SSLSocket) sslSocketFactory.createSocket(host, port)) {
            ...
            try (
                ...
                for (int i = 1; i <= numMessages; i++) {
                    String message = messagePrefix + " - Message #" + i + " - " + generatePayload(i);
                    ...
                    if (ack != null && ack.startsWith("ACK")) {
                        successCount++;
                        if (i % 10 == 0 || i <= 5) {
                            System.out.println("Message " + i + " - Latency: " + latencyMs + " ms - Response: " + ack);
                        }
                    } else {
                        System.err.println("Unexpected response for message " + i + ": " + ack);
                    }
                ...
                System.out.println("Successful acknowledgments: " + successCount);
                System.out.println("Average latency: " + (totalLatency / numMessages) + " ms");
     ...
     */
    public static void runLoadTest(String host, int port, String truststorePath, String truststorePassword,
                                    int numClients, int messagesPerClient) throws Exception {
        System.out.println("Starting load test with " + numClients + " concurrent clients...");
        ...
        for (int i = 0; i < numClients; i++) {
            ...
            new Thread(() -> {
                try {
                    ...
                    client.connect(truststorePath, truststorePassword, messagesPerClient, "Client-" + clientId);
        ...
        System.out.println("Throughput: " + (totalMessages.get() * 1000.0 / totalTime) + " messages/second");
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            if (loadTest) {
                runLoadTest(host, port, truststorePath, truststorePassword, numClients, numMessages);
            } else {
                BenchClient client = new BenchClient(host, port);
                client.connect(truststorePath, truststorePassword, numMessages, "SingleClient");
            }
        } catch (Exception e) {
    ...
    private static void printUsage() {
        ...
        System.out.println("  -m, --messages <count>         Number of messages to send (default: 100)");
        System.out.println("  -l, --load-test                Run load test with multiple clients");
        System.out.println("  -c, --clients <count>          Number of concurrent clients for load test (default: 10)");
        System.out.println("  --help                         Display this help message");
        System.out.println("\nExamples:");
        System.out.println("  Single client:");
        System.out.println("    java client.BenchClient --host localhost --port 8443 --messages 50");
        System.out.println("  Load test:");
        System.out.println("    java client.BenchClient --load-test --clients 20 --messages 100");
    }
}
